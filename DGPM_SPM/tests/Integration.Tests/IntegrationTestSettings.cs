...
public static class IntegrationTestSettings
{
    ...
    /// 2. 環境變數 <c>DGPM_SPM_INTEGRATION_CONNECTION</c>（僅測試用別名）
    /// </summary>
    public static string? ConnectionString =>
        FirstNonEmpty(
            Environment.GetEnvironmentVariable("ConnectionStrings__DefaultConnection"),
            Environment.GetEnvironmentVariable("DGPM_SPM_INTEGRATION_CONNECTION"));

    public static bool HasConnectionString => !string.IsNullOrWhiteSpace(ConnectionString);

    public const string DbSkipReason =
        "未設定 ConnectionStrings__DefaultConnection（或 DGPM_SPM_INTEGRATION_CONNECTION）；" +
