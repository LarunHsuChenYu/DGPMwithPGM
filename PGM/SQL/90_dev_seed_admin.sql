/* =============================================================================
   ...
   測帳：Admin（系統管理員→PGMAdmin＋DGPMAdmin）、AshtonHsu（另含 Uploader／Reviewer）
...
/* ---------- 2. 開發帳號（AshtonHsu＋Admin） ----------
   產品預期：Admin（系統管理員）在 PGM 有權 → 進 DGPM（systemCode=DGPM）須有業務選單，不可空側欄。
...
/* Admin＝系統管理員：PGM＋DGPM 各一管理者角色（進 DGPM 預設即 DGPMAdmin 全業務選單） */
...
/* AshtonHsu：聯調多角色切換（含 Uploader／Reviewer） */
INSERT INTO dbo.MAP_USER_ROLE (USER_ID, ROLE_ID, CRT_DATE, CRT_USER)
VALUES
...
(
    SELECT * FROM (VALUES
        ...
        /* 保留 Fun 定義（不進選單、不授權給 DGPM*）：必要時可手動開授權，一般請直接開 PGM Web */
        (N'Permission', N'系統權限管理', NULL, CAST(200 AS SMALLINT),
         N'M', N'N', N'N', N'已退役選單父層（帳號角色請至 PGM）', N'系統權限管理', NULL, N'DGPM'),
        (N'PgmAuthLink', N'帳號與角色維護', N'ext:pgm', CAST(220 AS SMALLINT),
         N'P', N'N', N'N', N'保留定義；不掛 DGPMAdmin。帳號／角色請直接開 PGM Web', N'系統權限管理', N'Permission', N'DGPM'),
        /* 5 Syslog */
...
/* Permission／PgmAuthLink：保留列但軟刪選單（MERGE 已設 IS_MENU=N；強制清掉殘留授權） */
UPDATE dbo.[SET_FUNCTION]
SET DEL_FLG = 1, IS_MENU = N'N', IS_ENABLED = N'N', MDF_DATE = @Now, MDF_USER = @SeedUser
WHERE FUNCTION_ID IN (N'Permission', N'PgmAuthLink')
...
/* PGMAdmin：所有 PGM 系統功能（含管理頁；不含 DGPM 業務） */
...
/* DGPMAdmin：全部啟用中的 DGPM 葉（不含 PgmAuthLink／已軟刪 Permission） */
INSERT INTO dbo.MAP_ROLE_FUNCTION (ROLE_ID, FUNCTION_ID, CRT_DATE, CRT_USER)
...
WHEN MATCHED THEN
    UPDATE SET T.SET_VALUE = S.SET_VALUE, T.SORT_ORDER = S.SORT_ORDER, T.DEL_FLG = 0,
               T.MDF_DATE = @Now, T.MDF_USER = @SeedUser
WHEN NOT MATCHED THEN
    INSERT (SET_ITEM, SET_ID, SET_VALUE, SORT_ORDER, MEMO, DEL_FLG, CRT_DATE, CRT_USER)
    VALUES (S.SET_ITEM, S.SET_ID, S.SET_VALUE, S.SORT_ORDER, N'', 0, @Now, @SeedUser);
