...
/// </summary>
public sealed class PgmApiFactory : WebApplicationFactory<Program>
{
    protected override void ConfigureWebHost(IWebHostBuilder builder)
    {
        var connectionString = IntegrationTestSettings.ConnectionString
            ?? throw new InvalidOperationException(
                "PgmApiFactory 需要 ConnectionStrings__DefaultConnection。");
        ...
        builder.ConfigureAppConfiguration((_, config) =>
        {
            config.AddInMemoryCollection(new Dictionary<string, string?>
            {
                ...
                ["JwtSettings:Issuer"] = "PGM.Api",
                ["JwtSettings:Audience"] = "PGM.Api",
